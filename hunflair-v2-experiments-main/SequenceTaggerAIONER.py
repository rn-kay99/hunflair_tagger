...
class SequenceTaggerAIONER(flair.nn.Classifier[Sentence]):
    def __init__(
        self,
        ...
        """
        super(SequenceTaggerAIONER, self).__init__()
        ...
        if init_from_state_dict:
            ...
            if tag_dictionary.span_labels:
                ...
                self.label_dictionary.add_item("O-All")
                for label in tag_dictionary.get_items():
                    if label == "<unk>":
                        continue
                    if self.tag_format == "BIOES":
                        self.label_dictionary.add_item("O-" + label)
        ...
        log.info(f"SequenceTaggerAIONER predicts: {self.label_dictionary}")
        ...
        if (
            ...
            and not self.label_dictionary.start_stop_tags_are_set()
        ):
        ...
        if use_crf:
            self.crf = CRF(
                self.label_dictionary, self.tagset_size, init_from_state_dict
            )
    ...
    def forward_loss(self, sentences: List[Sentence]) -> Tuple[torch.Tensor, int]:
        ...
        if len(sentences) == 0:
            return (
                torch.tensor(
                    0.0, dtype=torch.float, device=flair.device, requires_grad=True
                ),
                0,
            )
    ...
    def _calculate_loss(
        self, scores: torch.Tensor, labels: torch.LongTensor
    ...
    @staticmethod
    def _get_sentence_aioner_type(sentence: Sentence) -> str:
        """
        Extracts the AIONER type from the sentence.
        :param sentence: Sentence to extract AIONER type from
        """
        # TODO: Fix this hack to get around the fact that we don't have a
        #  proper way to store different "O" entity types in the data
        entity_types = []
        match = re.match("\[(.*?)\]", sentence.to_plain_string())
        if "genes" in match.group():
            entity_types.append("Gene")
        if "diseases" in match.group():
            entity_types.append("Disease")
        if "chemicals" in match.group():
            entity_types.append("Chemical")
        if "species" in match.group():
            entity_types.append("Species")
        if "cell lines" in match.group():
            entity_types.append("CellLine")
        if len(entity_types) == 1:
            ...
            raise ValueError(
                "No entity types found in sentence " + sentence.to_plain_string()
            )
    ...
    def _get_gold_labels(self, sentences: List[Sentence]) -> List[str]:
        ...
        if self.predict_spans:
            all_sentence_labels = []
            for sentence in sentences:
                sentence_type = SequenceTaggerAIONER._get_sentence_aioner_type(sentence)
                sentence_labels = ["O-" + sentence_type] * len(sentence)
        ...
        # all others are regular labels for each token
            ...
            labels = [
                token.get_label(
                    self.label_type,
                    "O-" + SequenceTaggerAIONER._get_sentence_aioner_type(sentence),
                ).value
                for sentence in sentences
        ...
        with torch.no_grad():
            ...
            for batch in dataloader:
                ...
                for sentence, sentence_predictions in zip(batch, predictions):
                    sentence_type = SequenceTaggerAIONER._get_sentence_aioner_type(
                        sentence
                    )
                    ...
                    # token-labels can be added directly ("O" and legacy "_" predictions are skipped)
                        ...
                        for token, label in zip(sentence.tokens, sentence_predictions):
                            if label[0] in ["O-" + sentence_type, "_"]:
                                continue
                            token.add_label(
    ...
    def _all_scores_for_token(
        self, sentences: List[Sentence], scores: torch.Tensor, lengths: List[int]
    ):
        """
...
from flair.models import SequenceTaggerAIONER

# load tagger
tagger = SequenceTaggerAIONER.load("{repo_id}")
...
```"""
    ...
    def push_to_hub(
        ...
        private: bool = None,
        commit_message: str = "Add new SequenceTaggerAIONER model.",
    ):
        """
...
def get_spans_from_bio(
    bioes_tags: List[str], bioes_scores=None
    ...
    for idx, bioes_tag in enumerate(bioes_tags):
        ...
        in_span = False if bioes_tag.startswith("O-") else True
        ...
        if (starts_new_span or not in_span) and len(current_span) > 0:
            ...
            span_value = max(
                current_tag_weights.keys(), key=current_tag_weights.__getitem__
            )
