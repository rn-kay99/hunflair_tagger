...
def venn4(labels, names=['A', 'B', 'C', 'D'], **options):
    ...
    leg = ax.legend(names, loc='lower center', ncols=4, bbox_to_anchor=(0.5, 0.0), fancybox=True)
    leg.get_frame().set_alpha(0.5)
...
def venn5(labels, names=['A', 'B', 'C', 'D', 'E'], **options):
    ...
    # legend
    draw_text(fig, ax, 0.02, 0.72, names[0], colors[0], fontsize=fontsize_leg, ha="right")
    draw_text(fig, ax, 0.72, 0.94, names[1], colors[1], fontsize=fontsize_leg, va="bottom")
    draw_text(fig, ax, 0.90, 0.74, names[2], colors[2], fontsize=fontsize_leg, ha="left")
    draw_text(fig, ax, 0.88, 0.05, names[3], colors[3], fontsize=fontsize_leg, ha="left")
    draw_text(fig, ax, 0.12, 0.05, names[4], colors[4], fontsize=fontsize_leg, ha="right")
    # leg = ax.legend(names, loc='center left', bbox_to_anchor=(1.0, 0.5), fancybox=True)
    # leg.get_frame().set_alpha(0.5)
