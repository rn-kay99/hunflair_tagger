...
class GeneticVariantTokenizer(Tokenizer):
    """Tokenizer based on space character, digits, or common punctuation."""
    PATTERN = r'inv|del|ins|dup|tri|qua|con|delins|indel|fsX|fsx|fs x|fs|\d+|[a-zA-Z]+|[%#_\.\*\+\-&,~!:;=\/<>\?\[\]\(\)\{\}\"\']' 

    def __init__(self) -> None:
        super().__init__()

    def tokenize(self, text: str) -> List[str]:
        return GeneticVariantTokenizer.run_tokenize(text)

    @staticmethod
    def run_tokenize(text: str) -> List[str]:
        return re.findall(GeneticVariantTokenizer.PATTERN, text)
