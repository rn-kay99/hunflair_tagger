...
from flair.splitter import (
    ...
    SentenceSplitter,
    TagSentenceSplitter,
    GeneticVariantSentenceSplitter,
)
...
GENE_TAG = "Gene"
SPECIES_TAG = "Species"
GENETIC_VARIANT_TAG = "GeneticVariant"

# for training model to learn genetic variants identifier
REFERENCE_SEQUENCE_TYPE_TAG = "ReferenceSequenceType"
DNA_RNA_NUCLEOTIDE_TAG = "DnaRnaNucleotide"
MUTATION_UNIT_TAG = "MutationUnit"
MUTATION_TYPE_TAG = "MutationType"
PROTEIN_AMINO_ACID_TAG = "ProteinAminoAcid"
FRAME_SHIFT_MUTATION_TAG = "FrameShiftMutation"
SNP_ID_TAG = "SnpId"
EXON_INTRON_TAG = "ExonIntron"
MUTATION_COMPONENT_TAG = "MutationComponent"
...
class GENETIC_VARIANT_NER_CORPUS(BIGBIO_NER_CORPUS):
    def get_entity_type_mapping(self) -> Optional[Dict]:
        """Return the mapping of entity type given in the dataset to canonical types.

        Note, if a entity type is not present in the map it is discarded.
        """
        return {"variant": GENETIC_VARIANT_TAG, "ProteinMutation": GENETIC_VARIANT_TAG, "DNAMutation": GENETIC_VARIANT_TAG, "SNP": GENETIC_VARIANT_TAG, "SequenceVariant": GENETIC_VARIANT_TAG}
    
class FINE_GRAINED_GENETIC_VARIANT_NER_CORPUS(ColumnCorpus):
    def __init__(
        self,
        dataset_name: str,
        base_path: Optional[Union[str, Path]] = None,
        ...
        self.sentence_splitter = sentence_splitter if sentence_splitter else GeneticVariantSentenceSplitter()

        # get corpora path
        base_path = Path.home() / ".flair" / "datasets"
        data_folder = base_path / dataset_name
        train_json_file = data_folder / "json_data/train.json"
        test_json_file = data_folder / "json_data/test.json"

        splits = {}
        # dev_split_name = "validation" # not used currently

        try:
            with open(train_json_file, "r") as file:
                file_content = file.read()
                fine_grained_data = json.loads(file_content)
                splits["train"] = self.to_internal_dataset(fine_grained_data, "train")
        except FileNotFoundError:
            print(f"Die Datei '{train_json_file}' wurde nicht gefunden.")
        ...
        try:
            with open(test_json_file, "r") as file:
                file_content = file.read()
                fine_grained_data = json.loads(file_content)
                splits["test"] = self.to_internal_dataset(fine_grained_data, "test")
        except FileNotFoundError:
            print(f"Die Datei '{test_json_file}' wurde nicht gefunden.")
        except Exception as e:
            print(f"Ein Fehler ist aufgetreten: {e}")
        ...
        super().__init__(data_folder, columns, in_memory=in_memory, comment_symbol="#")

    def get_entity_type_mapping(self) -> Optional[Dict]:
        return {"mutation_unit": MUTATION_UNIT_TAG, "mutation_type": MUTATION_TYPE_TAG, "mutation_component": MUTATION_COMPONENT_TAG, "dna_rna_nucleotide": DNA_RNA_NUCLEOTIDE_TAG, "reference_sequence_type": REFERENCE_SEQUENCE_TYPE_TAG, "protein_amino_acid": PROTEIN_AMINO_ACID_TAG, "frame_shift_mutation": FRAME_SHIFT_MUTATION_TAG, "snp_id": SNP_ID_TAG, "exon_intron": EXON_INTRON_TAG}
    ...
    def to_internal_dataset(self, dataset, split: str) -> InternalBioNerDataset:
        ...
        for key, value in dataset.items():
            ...
            entities_key = f"{key}_entities"
            id_to_text[f"{key}_title"] = value[text_key]

            id_to_entities[f"{key}_title"] = []
            for entity_str in value[entities_key]:
                # Extrahiere die char_span und entity_type aus dem String
                start, end = map(int, entity_str.split("(")[1].split(")")[0].split(","))
                entity_type = entity_str.split("(")[0]

                # Erstelle ein neues Entity-Objekt und füge es zur Liste hinzu
                entity = Entity(char_span=(start, end), entity_type=entity_type)
                id_to_entities[f"{key}_title"].append(entity)
...
class HUNER_GENETIC_VARIANT_OSIRIS(BIGBIO_NER_CORPUS):
    def __init__(
        self,
        base_path: Optional[Union[str, Path]] = None,
        in_memory: bool = True,
    ...
    def get_entity_type_mapping(self) -> Optional[Dict]:
        return {"variant": GENETIC_VARIANT_TAG}

    def build_corpus_directory_name(self, dataset_name: str) -> str:
        return self.__class__.__name__.lower()   


class HUNER_GENETIC_VARIANT(HunerMultiCorpus):
    """Union of all HUNER cell line data sets."""

    def __init__(self, sentence_splitter: Optional[SentenceSplitter] = None) -> None:
        super().__init__(entity_type="GENETIC_VARIANT", sentence_splitter=sentence_splitter)

class HUNER_GENETIC_VARIANT_BIORED(BIGBIO_NER_CORPUS):
    def __init__(
        self,
        base_path: Optional[Union[str, Path]] = None,
        in_memory: bool = True,
    ...
    def get_entity_type_mapping(self) -> Optional[Dict]:
        return {"SequenceVariant": GENETIC_VARIANT_TAG}

    def build_corpus_directory_name(self, dataset_name: str) -> str:
        return self.__class__.__name__.lower()

class HUNER_GENETIC_VARIANT_TMVAR_V3(BIGBIO_NER_CORPUS):
    def __init__(
        self,
        base_path: Optional[Union[str, Path]] = None,
        in_memory: bool = True,
    ...
    def get_entity_type_mapping(self) -> Optional[Dict]:
        return {"ProteinMutation": GENETIC_VARIANT_TAG, "DNAMutation": GENETIC_VARIANT_TAG, "SNP": GENETIC_VARIANT_TAG}
